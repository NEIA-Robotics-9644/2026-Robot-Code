...
public class ShootWhenAtSpeedPercent extends Command {
    ...
    public ShootWhenAtSpeedPercent(Shooter shooter, DoubleSupplier rollerNormalizedVelocity, 
        DoubleSupplier rollerFeedForward, DoubleSupplier percentOffToOnThreshold, 
    ...
    public void execute() {
        ...
        } else if (leftFlywheelPerecentage < percentOnToOffThreshold.getAsDouble() && leftFlywheelRunning) {
        ...
        } else if (rightFlywheelPerecentage < percentOnToOffThreshold.getAsDouble() && rightFlywheelRunning) {
        ...
        */
        if (leftFlywheelRunning) {
            shooter.setFlywheelVelocity(rollerNormalizedVelocity.getAsDouble(), 
                rollerFeedForward.getAsDouble(), FlywheelSide.LEFT_ROLLERS);
        }
        else{
            shooter.setFlywheelVelocity(0, 
                0, FlywheelSide.LEFT_ROLLERS);
        }

        if (rightFlywheelRunning) {
            shooter.setFlywheelVelocity(rollerNormalizedVelocity.getAsDouble(), 
                rollerFeedForward.getAsDouble(), FlywheelSide.RIGHT_ROLLERS);
        }
        else{
            shooter.setFlywheelVelocity(0, 
                0, FlywheelSide.RIGHT_ROLLERS);
        }
    }
