import re
import subprocess
repo = "/Users/vishnubharath/Documents/GitHub/FRC/2026-Robot-Code"
def git(*args):
    return subprocess.check_output(["git", "-C", repo, *args], text=True).strip()
exclude = {"refactor-lib", "refactor-robot-vs-lib"}
branches = [b for b in git("for-each-ref", "--format=%(refname:short)", "refs/heads").splitlines() if b and b not in exclude]
...
branch_commits = {b: set(git("rev-list", b).splitlines()) for b in branches}
main_first_parent = set(git("log", "--first-parent", "--pretty=format:%H", "main").splitlines()) if "main" in branches else set()
main_commits = branch_commits.get("main", set())
lane_index = {b: i for i, b in enumerate(lane_order)}
def distance_to_tip(branch: str, sha: str) -> int:
    return int(git("rev-list", "--count", f"{sha}..{branch}"))
def choose_branch(sha: str) -> str:
...
for sha in commits:
    ...
    for parent in parents_map.get(sha, []):
        ...
        parts.append(
            f"<path fill=\"none\" stroke=\"{color}\" stroke-width=\"1.5\" stroke-linecap=\"round\" stroke-linejoin=\"round\" d=\"{d}\" marker-end=\"{marker}\" />"
        )
parts.append("</g>")
parts.append("")
